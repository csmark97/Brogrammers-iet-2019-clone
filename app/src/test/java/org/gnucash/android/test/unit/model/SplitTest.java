...
public class SplitTest {
   ...
   /*  @Test
    public void shouldClone() {
        Split split = new Split(new Money("3", "USD"), new Money("2", "EUR"), "account-UID");
        Split clone = split.clone();
        assertThat(split.getValue().getNumerator()).isEqualTo(clone.getValue().getNumerator());
        assertThat(split.getUID()).isEqualTo(clone.getUID());
    } */

    /* @Test
    public void shouldFormatValuesCorrectlyDebit() {
        Money money = new Money("10", "EUR");
        Split split = new Split(new Money("10", "EUR"), "account-UID");
        split.setType(TransactionType.DEBIT);
        split.setValue(money);
        split.setAccountUID("account-UID");

        long money2 = split.getFormattedValue().getNumerator();

        assertThat(money2).isEqualTo(money.getNumerator());
    } */

    @Test
    public void testReconciled() {
        Split split = new Split(new Money("10", "EUR"), "account-UID");
        split.setReconcileState('y');

        assertThat(split.isReconciled()).isEqualTo(true);
    }

    @Test
    public void shouldParseSmallerCsv(){
        String csv = "100;USD;account;mytransactionUID;CREDIT;memo";
        Split split = Split.parseSplit(csv);
        assertThat(split.getValue().getNumerator()).isEqualTo(new Money("100", "USD").getNumerator());
        assertThat(split.getTransactionUID()).isEqualTo("mytransactionUID");
        assertThat(split.getAccountUID()).isEqualTo("account");
        assertThat(split.getType()).isEqualTo(TransactionType.CREDIT);
        assertThat(split.getMemo()).isEqualTo("memo");
    }

    @Test
    public void hashShouldNotBeZero(){
        Split split = new Split(new Money("10", "EUR"), "account-UID");

        assertThat(split.hashCode()).isNotEqualTo(0);
    }
}
